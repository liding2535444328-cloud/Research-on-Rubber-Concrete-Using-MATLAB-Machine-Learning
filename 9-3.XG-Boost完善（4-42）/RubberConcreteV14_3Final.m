...
%% ================== 核心逻辑函数 ==================
    ...
    function val_out = get_val_internal(x_input_vector)
        % 终极物理级隔离预测助手
        try
            % 维度强制重塑
            x_ready = double(reshape(x_input_vector, 1, 9));
            px_n = mapminmax('apply', x_ready', Final_PS_In)';
            
            % --- 暴力尝试三种调用路径，彻底封死点索引报错 ---
            try
                % A 路径：方法句柄（最稳健）
                ty_n = RC_PREDICT_Method(Final_Model_Object, px_n);
            catch
                try
                    % B 路径：feval 字符强制检索
                    ty_n = feval('PREDICT', Final_Model_Object, px_n);
                catch
                    % C 路径：直接对象属性访问
                    ty_n = PREDICT(Final_Model_Object, px_n);
                end
            end
            
            val_out = mapminmax('reverse', ty_n', Final_PS_Out);
            
        catch ME_inner
            % 如果还是报错，将底层错误抛回给顶层诊断框
            rethrow(ME_inner);
        end
    end
end