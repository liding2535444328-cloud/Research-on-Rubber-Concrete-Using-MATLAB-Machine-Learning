%% ========================================================================
%  项目：橡胶混凝土强度预测系统 (PSO-LSBoost 9输入稳健增强版)
%  数据：数据集3 (9个特征输入 -> 1个强度输出)
%  核心优化：1. R2_train 报错修复 2. 8输入版架构同步 3. 诊断保存加强
%% ========================================================================
warning off; clear; clc; close all;
%% --- 模块 0: 开启并行计算 (针对 i5-10200H 优化) ---
...
parfor run_i = 1:loop_num
    ...
    % 3. PSO 微调 (针对 9 输入加速)
    pop = 10; max_it = 15; lb = [0.05, 3]; ub = [0.3, 10];
    x = lb + (ub-lb).*rand(pop,2); v = zeros(pop,2);
    pB = x; pB_s = inf(pop,1); gB = x(1,:); gB_s = inf; cur_c = zeros(1, max_it);
    
    for t = 1:max_it
        for i = 1:pop
            t_tree = templateTree('MaxNumSplits', round(x(i,2)));
            m = fitrensemble(p_tr_n', t_tr_n', 'Method', 'LSBoost', 'NumLearningCycles', 40, 'LearnRate', x(i,1), 'Learners', t_tree);
    ...
    final_m = fitrensemble(p_tr_n', t_tr_n', 'Method', 'LSBoost', 'NumLearningCycles', 80, 'LearnRate', gB(1), 'Learners', best_t);
