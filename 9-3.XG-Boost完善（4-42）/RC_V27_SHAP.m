function RC_V36_SHAP_Integrated_Platform()
% =====================================================================
% 项目：橡胶混凝土 AI 智能设计系统 (V36 SHAP优化集成版)
% 核心：[左] V33精美SHAP + [中] V35星簇监控 + [右] 敏感性轨迹
% 特色：全中英双语、九色联动、极速响应
% =====================================================================
...
% 坐标轴分配
axSHAP = axes(pnlDisp1, 'Position', [0.04 0.15 0.25 0.72]); % 左：V33 风格 SHAP
ax3D = axes(pnlDisp1, 'Position', [0.35 0.1 0.35 0.85]);    % 中：V35 星簇
axHist = axes(pnlDisp1, 'Position', [0.73 0.15 0.24 0.72]);  % 右：趋势轨迹
...
%% ================== [界面 2: 逆向智能寻优] ==================
pnlCfg2 = uipanel(tabInverse, 'Title', 'Target Config & Optimization', 'Position', [0.02 0.72 0.96 0.25]);
pnlPlot2 = uipanel(tabInverse, 'Title', 'Evolution Trajectory (Physical)', 'Position', [0.02 0.35 0.96 0.35]);
pnlData2 = uipanel(tabInverse, 'Title', 'Optimized Mix Design', 'Position', [0.02 0.02 0.96 0.32]);
...
resTable2 = uitable(pnlData2, 'Units', 'normalized', 'Position', [0.01 0.05 0.98 0.9], ...
                   'ColumnName', {'Component', 'Value', 'Unit', 'Range', 'Validation'}, 'ColumnWidth', {150, 80, 60, 150, 500});
...
%% ================== 核心逻辑函数 ==================
    ...
    function update_forward(idx, mode)
        ...
        % --- 调用整合后的绘图模块 ---
        draw_SHAP_V33(X_curr, Y_curr); % 左：V33 风格 SHAP
        draw_StarCluster_V35(X_curr, Y_curr); % 中：V35 星簇监控
        draw_History_V35(); % 右：记录轨迹
        drawnow;
    end

    % 【左侧】：保留 V33 精美风格的 SHAP
    ...
    function run_inverse_v36()
        try
            ...
            for s = 1:size(seeds, 1)
                ...
                obj = @(x) 1000 * (get_pred_quick(x) - target)^2; 
            ...
            for k = 1:9
                ...
                tbl{k,1} = fNamesEN{k}; tbl{k,2} = sprintf('%.2f', val);
