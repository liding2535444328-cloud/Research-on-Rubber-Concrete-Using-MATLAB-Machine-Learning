...
try
    ...
    % --- 核心加固：建立底层预测句柄 ---
    % 注意：这里统一命名为 RC_Predict_Method
    RC_Predict_Method = @classreg.learning.ensemble.predict;
...
%% ================== 核心逻辑函数 ==================
    ...
    function robust_prediction_v7()
        try
            X_val = zeros(1, 9);
            for k = 1:9; X_val(k) = str2double(get(predEditHandles{k}, 'String')); end

            % 执行带异常处理的预测
            Y_res_p = get_val_internal(X_val);

            axes(axPredict); cla; hold on;
            bar(1, Y_res_p, 0.5, 'FaceColor', [0.2 0.5 0.7], 'EdgeColor', 'k');
            errorbar(1, Y_res_p, Final_MAE_Value, 'LineWidth', 2, 'Color', 'r');
            ylabel('强度 (MPa)'); set(gca, 'XTick', []);
            title('预测强度结果 (含 MAE 误差棒)'); grid on;
            set(lblRes, 'String', sprintf('预测值: %.2f MPa | 理论误差范围: ±%.2f MPa', Y_res_p, Final_MAE_Value));
        catch ME
            err_msg = {['推理失败原因：', ME.message], '', ...
                '【点索引错误专解建议】：', ...
                '1. 重点排查：请检查当前文件夹是否有 predict.m，若有请重命名！', ...
                '2. 输入 whos predict 检查：若返回变量，请执行 clear predict。', ...
                '3. 检查模型状态：请在命令行输入 class(Final_Model_Object) 确认是否为对象。'};
            errordlg(err_msg, '核心报错诊断');
        end
    end
    ...
    function val_out = get_val_internal(x_dosage)
        % 终极物理级隔离预测逻辑
        try
            % 维度预处理
            xt_p = double(reshape(x_dosage, 1, 9));
            px_p = mapminmax('apply', xt_p', Final_PS_In)';

            % --- 绝杀修复：尝试三种暴力预测路径 ---
            try
                % 路径 A: 使用主函数预绑定的句柄 (最推荐)
                ty_p = RC_Predict_Method(Final_Model_Object, px_p);
            catch
                try
                    % 路径 B: 强制 feval 字符调用
                    ty_p = feval('predict', Final_Model_Object, px_p);
                catch
                    % 路径 C: 底层类路径直接调用
                    ty_p = classreg.learning.ensemble.predict(Final_Model_Object, px_p);
                end
            end

            val_out = mapminmax('reverse', ty_p', Final_PS_Out);

        catch ME
            % 针对“点索引”错误的特殊诊断抛出
            if contains(ME.message, '点进行索引')
                error('检测到环境冲突：请在命令行输入 clear all 并删除文件夹下的 predict.m');
            else
                rethrow(ME);
            end
        end
...
end % 这是全程序的最后一个 END，确保它闭合了最开头的 function
