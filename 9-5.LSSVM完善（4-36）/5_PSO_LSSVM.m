%% ========================================================================
%  项目：橡胶混凝土强度预测系统 (PSO-LSSVM 9输入并行科研最终版)
%  数据：数据集3 (9个特征输入 -> 1个强度输出)
%  核心：1. 并行加速(PSO)  2. LSSVM 算法  3. MAE全覆盖  4. SHAP分析  5. 标题下置
%% ========================================================================
warning off; clear; clc; close all;
% 确保路径下存在 LSSVM 工具箱
if exist('LSSVM_Toolbox', 'dir'), addpath('LSSVM_Toolbox\'); end 
rng('shuffle'); 
...
%% --- 模块 3: 执行稳定性循环实验 (含 PSO 寻优) ---
fprintf('正在执行 %d 次 PSO-LSSVM 训练与稳定性测试...\n', loop_num);
best_R2_val = -inf;
...
for run_i = 1:loop_num
    ...
    % PSO 寻优 LSSVM 参数 (gam, sig2)
    pop_size = 20; max_gen = 25; lb = [0.1, 0.01]; ub = [1000, 100];
    part = lb + (ub - lb) .* rand(pop_size, 2); vel = zeros(pop_size, 2);
    pB = part; pB_sc = inf(pop_size, 1); gB = part(1,:); gB_sc = inf; cur_c = zeros(1, max_gen);
    
    for g = 1:max_gen
        for i = 1:pop_size
            try fit = crossvalidate({p_train', t_train', 'f', part(i,1), part(i,2), 'RBF_kernel'}, 5); 
            catch; fit = 1e6; end
            if fit < pB_sc(i); pB_sc(i) = fit; pB(i,:) = part(i,:); end
            if fit < gB_sc; gB_sc = fit; gB = part(i,:); end
        end
        vel = 0.7*vel + 1.2*rand*(pB - part) + 1.2*rand*(repmat(gB, pop_size, 1) - part);
        part = part + vel; part = max(min(part, ub), lb); cur_c(g) = gB_sc;
    end
    
    % 最终训练与预测
    [alpha, b_bias] = trainlssvm({p_train', t_train', 'f', gB(1), gB(2), 'RBF_kernel'});
    T_sim1 = mapminmax('reverse', simlssvm({p_train', t_train', 'f', gB(1), gB(2), 'RBF_kernel'}, {alpha, b_bias}, p_train')', ps_out);
    T_sim2 = mapminmax('reverse', simlssvm({p_train', t_train', 'f', gB(1), gB(2), 'RBF_kernel'}, {alpha, b_bias}, p_test')', ps_out);
    ...
    if stats_R2(run_i) > best_R2_val
        best_R2_val = stats_R2(run_i);
        bp.model = {alpha, b_bias, gB}; bp.conv = cur_c;
        ...
        bp.p_train = p_train; bp.p_test = p_test; bp.ps_in = ps_in; bp.ps_out = ps_out;
    end
end
...
annotation('textbox', [0.58, 0.68, 0.12, 0.15], 'String', ...
    {['R^2 = ', num2str(bp.R2_test, '%.4f')], ['RMSE = ', num2str(bp.rmse2, '%.3f')], ['MAE = ', num2str(bp.mae2, '%.3f')]}, ...
...
text(0.5, 0.1, {'表1: PSO-LSSVM 模型性能评估统计报表'; 'Table 1: Performance Evaluation Report'}, 'Units', 'Normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
...
grid on; xlabel('进化代数'); ylabel('适应度 (CV-RMSE)'); set(gca, 'Position', [0.15, 0.22, 0.75, 0.7]);
text(0.5, -0.22, {'图5: PSO 参数深度寻优收敛曲线'; 'Fig.5: PSO Hyperparameter Optimization Search'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

% 图6 & 图7: 训练/测试集相关性
drawScatter(bp.T_train, bp.T_sim1, [0.15 0.4 0.15], '图6: 训练集预测分布图', 'Fig.6: Training Set Correlation');
drawScatter(bp.T_test, bp.T_sim2, [0.5 0.15 0.15], '图7: 测试集预测分布图', 'Fig.7: Testing Set Correlation');

%% --- 模块 5: 稳定性统计与特征重要性 (图8-11) ---
...
annotation('textbox', [0.3, 0.02, 0.4, 0.1], 'String', {'图8-10: PSO-LSSVM 模型稳定性统计评估 '; 'Fig.8-10: Stability Distribution of Metrics '}, 'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontWeight', 'bold', 'FontSize', 11);

% 置换检验计算特征重要性
fprintf('正在计算置换重要性...\n');
base_err = bp.mae2; importance = zeros(1, 9);
for f = 1:9
    p_perm = bp.p_test; p_perm(f, :) = p_perm(f, randperm(size(p_perm, 2)));
    t_perm = mapminmax('reverse', simlssvm({bp.p_train', bp.p_train(1,:)', 'f', bp.model{3}(1), bp.model{3}(2), 'RBF_kernel'}, ...
             {bp.model{1}, bp.model{2}}, p_perm')', bp.ps_out);
    importance(f) = abs(mean(abs(t_perm - bp.T_test')) - base_err);
end
rel_imp = (importance / sum(importance)) * 100; [sorted_imp, idx] = sort(rel_imp, 'ascend');
...
text(0.5, -0.2, {'图11: 基于置换检验的 9 维特征重要性分析'; 'Fig.11: 9-Dimensional Feature Importance Analysis'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

%% --- 模块 6: SHAP 解释性机理分析 (图12) ---
fprintf('正在执行 SHAP 贡献价值模拟分析...\n');
...
text(0.5, -0.22, {'图12: PSO-LSSVM 模型 SHAP 特征影响摘要图'; 'Fig.12: SHAP Summary Plot for LSSVM Model'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

%% --- 模块 8: 一键自动保存所有图片 ---
fprintf('正在自动导出所有科研图表...\n');
if ~exist('LSSVM_Figures', 'dir'); mkdir('LSSVM_Figures'); end
figH = findobj('Type', 'figure');
for i_fig = 1:length(figH)
    saveName = fullfile('LSSVM_Figures', sprintf('Fig_%d.png', figH(i_fig).Number));
    exportgraphics(figH(i_fig), saveName, 'Resolution', 300);
end
fprintf('✅ 全部 12 张高清科研图表已导出至 LSSVM_Figures 文件夹。 \n');
