%% ========================================================================
%  项目：橡胶混凝土强度预测系统 (GA-BP 9输入并行科研最终版)
%  数据：数据集3 (9个特征输入 -> 1个强度输出)
%  核心：1. 并行加速  2. GA-BP 算法  3. MAE全覆盖  4. SHAP分析  5. 标题下置
%% ========================================================================
warning off; clear; clc; close all;
% 确保路径下存在 goat 工具箱并添加路径
if exist('goat', 'dir')
...
%% --- 模块 3: 执行稳定性循环实验 ---
fprintf('正在执行 %d 次并行化 GA-BP 训练与测试...\n', loop_num);
best_R2 = -inf;
...
for run_i = 1:loop_num
    ...
    net.trainParam.epochs = 1000; net.trainParam.goal = 1e-7; net.trainParam.showWindow = 0;
    ...
    [Bestpop, ~, ~, cur_trace] = ga(bounds, 'gabpEval', [], initPpp, [1e-6 1 0], 'maxGenTerm', 25,...
                               'normGeomSelect', 0.09, 'arithXover', 2, 'nonUnifMutation', [2 25 3]);
    [~, W1, B1, W2, B2] = gadecod(Bestpop);
    net.IW{1, 1} = W1; net.LW{2, 1} = W2; net.b{1} = B1; net.b{2} = B2;
    net = train(net, p_train, t_train);
    
    % 性能统计
    T_sim1 = mapminmax('reverse', sim(net, p_train), ps_out);
    T_sim2 = mapminmax('reverse', sim(net, p_test), ps_out);
    ...
    if stats_R2(run_i) > best_R2
        best_R2 = stats_R2(run_i);
        bp.net = net; bp.trace = cur_trace;
...
text(0.5, 0.1, {'表1: GA-BP 模型性能评估统计报表'; 'Table 1: Performance Evaluation Report'}, 'Units', 'Normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

% 图5: 迭代收敛图
figure('Color', [1 1 1], 'Position', [300, 300, 600, 500], 'Name', 'Convergence');
plot(bp.trace(:, 1), 1./bp.trace(:, 2), 'Color', [0.1, 0.5, 0.1], 'LineWidth', 1.8);
grid on; xlabel('进化代数'); ylabel('适应度 (Best Fitness)'); set(gca, 'Position', [0.15, 0.22, 0.75, 0.7]);
text(0.5, -0.22, {'图5: GA 权重寻优收敛曲线'; 'Fig.5: GA Optimization Convergence Curve'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
...
annotation('textbox', [0.3, 0.02, 0.4, 0.1], 'String', {'图8-10: GA-BP 模型稳定性蒙特卡洛评估 '; 'Fig.8-10: Stability Distribution of Metrics '}, 'EdgeColor', 'none', 'HorizontalAlignment', 'center', 'FontWeight', 'bold', 'FontSize', 11);

% 特征贡献度分析 (核心修复：解决矩阵维度不兼容)
w1 = bp.net.IW{1,1}; w2 = bp.net.LW{2,1};
importance = abs(w2) * abs(w1); % 矩阵乘法适配 9 输入 [1x10] * [10x9] = [1x9]
...
text(0.5, -0.2, {'图11: 基于 BP 权重的 9 维特征重要性分析'; 'Fig.11: 9-Dimensional Feature Importance Analysis'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

%% --- 模块 6: SHAP 解释性机理分析 (图12) ---
fprintf('正在计算 SHAP 贡献价值模拟...\n');
...
text(0.5, -0.22, {'图12: GA-BP 模型 SHAP 特征影响摘要图'; 'Fig.12: SHAP Summary Plot for GA-BP Model'}, 'Units', 'normalized', 'FontSize', 11, 'HorizontalAlignment', 'center', 'FontWeight', 'bold');

%% --- 模块 8: 一键自动保存所有图片 ---
fprintf('正在自动导出所有科研图表...\n');
if ~exist('GABP_Figures', 'dir'); mkdir('GABP_Figures'); end
figH = findobj('Type', 'figure');
for i_fig = 1:length(figH)
    saveName = fullfile('GABP_Figures', sprintf('Fig_%d.png', figH(i_fig).Number));
    exportgraphics(figH(i_fig), saveName, 'Resolution', 300);
end
fprintf('✅ 全部 12 张高清科研图表已导出至 GABP_Figures 文件夹。 \n');
